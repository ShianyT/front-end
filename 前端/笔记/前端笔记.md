...
<img src="images\echarts/pinia_example.gif" alt="pinia_example" style="zoom:30%;border:3px solid" /> 
...
<img src="images\echarts/image-20231119185900990.png" alt="image-20231119185900990" style="zoom:60%;" /> 
...
含义：把数据以图表这种更直观的方式进行呈现，让数据与数据之间的关系一目了然。
...
* `xAxis`
  ...
  直角坐标系 中的 x 轴, 如果type属性的值为category,那么需要配置data数据, 代表在x轴的呈现

* `yAxis`

  直角坐标系 中的 y 轴, 如果type属性配置为value, 那么无需配置data, 此时y轴会自动去series下找数据进行图表的绘制

* `series`

  系列列表。每个系列通过 type 决定自己的图表类型, data来设置每个系列的数据

配置项都是以键值对的形式存在, 并且配置项有很多, ECharts的学习大多是针对于这些配置项的,可以去其官网查找相关配置：`https://echarts.apache.org/zh/option.html`

## 3.`Echarts`常用图表

### 3.1【通用配置】

使用ECharts绘制出来的图表, 都天生就自带一些功能, 这些功能是每一个图表都具备的, 我们可以通过配置, 对这些功能进行设置.
...
tooltip指的是当鼠标移入到图表或者点击图表时, 展示出的提示框
...
toolbox是ECharts提供的工具栏, 内置有`导出图片、数据视图、重置、数据区域缩放、动态类型切换`五个工具
工具栏的按钮是配置在feature的节点之下
...
![9](images\echarts/9.png)

#### 3.1.4【图例：legend】

legend是图例，用于筛选类别，点击所需类别则可以单独或组合显示类别，需要和series配合使用
...
<body>
    ...
    <script>
        let mchart = echarts.init(document.querySelector('div'))
...
* 柱宽度`barWidth`
  ...
  效果图：![5](images\echarts/5.png)

* 横向柱状图

  让x轴的角色和y轴的角色互换一下即可. 既xAxis的type设置为value, yAxis的type设置为category, 并且设置data即可
...
<body>
    ...
    <script>
        ...
        const yDataArr = [3000, 2800, 900, 1000, 800, 700, 1400, 1300, 900, 1000, 800, 600]
        let option = {
            xAxis: {
                type: 'category',
                data: xDataArr,
...
* 线条样式`lineStyle`
  ...
  var option = {
   ...
              lineStyle: {
                  color: 'green',
                  type: 'dashed' // 可选值 dashed dotted solid
              }
          }
      ]
  }
...
* 如果每一组数据之间相差较少, 且都比0大很多, 那么有可能会出现这种情况：
  ...
  ![17](images\echarts\17.png)

  这显然不是我们想要的效果, 因此可以配置上scale, 让其摆脱0值比例
...
* `scale`配置
  ...
  ![18](images\echarts\18.png)

##### 堆叠图

* 堆叠图指的是, 同个类目轴上系列配置相同的stack值后，后一个系列的值会在前一个系列的值上相加，如果在一个图表中有两个或者多个折线图, 在没有使用堆叠配置的时候, 效果如下：
...
![21](images\echarts/21.png)

#### 3.4.2【散点图的常见效果】

##### 气泡图效果

气泡图效果：让每一个散点大小不同，颜色不同

* `symbolSize`控制散点大小
* `itemStyle.color`控制散点颜色

两者都支持固定值写法，也支持回调函数写法
...
* 回调函数的写法：
  ...
  ![23](images\echarts\23.png)

##### 涟漪动画效果

* `type:effectScatter`

  将type的值从`scatter`设置为`effectScatter`就能够产生涟漪动画的效果
...
* `rippleEffect`
  ...
  ![24png](images\echarts\24png.png)

* `showEffectOn`

  控制涟漪动画什么时候产生

  * `render`界面渲染完成就开始涟漪动画
  * `emphasis`鼠标移过某个散点就开始涟漪动画
...
* 圆环
  ...
  * `Array.`：数组的第一项是内半径，第二项是外半径, 通过Array, 可以将饼图设置为圆环图
    ...
    <script>
      ...
      $.get('json/map/china.json', function (chinaJson) {
        ...
        var option = {
          geo: {
            type: 'map',
            map: 'chinaMap', // 需要与registerMap中的第一个参数保持一致
          },
        }
        mCharts.setOption(option)
      })
...
* 显示某个区域
  ...
  ```js
      <script>
        var mCharts = echarts.init(document.querySelector('div'))
        // 获取该区域矢量数据
        $.get('json/map/province/guangdong.json', function (chinaJson) {
          // 注册json数据
          echarts.registerMap('guangdong', chinaJson)
          var option = {
            geo: {
              type: 'map',
              map: 'guangdong', // 需要与registerMap中的第一个参数保持一致
            },
          }
          mCharts.setOption(option)
        })
...
* 区域面积`areaStyle`
  ...
  ![35](images\echarts\35.png)

* 绘制类型`shape`

  雷达图绘制类型，支持 'polygon多边形' 和 'circle圆形' 
  ...
  ![36](images\echarts\36.png)

#### 3.7.3【雷达图的特点】

雷达图可以用来分析**多个维度**的数据与标准数据的对比情况
...
* 自定义主题
  ...
  * 在官网的主题编辑器中编辑主题：`https://echarts.apache.org/zh/theme-builder.html`
  ...
  * 在init方法中使用主题
    ...
    `var mCharts = echarts.init(document.querySelector("div"), '主题名')`

    这里的主题名称是在主题js文件中的代码中得到

    ![39](images\echarts\39.png)
...
* 渐变颜色的实现
  ...
  在ECharts中, 支持线性渐变和径向渐变两种颜色渐变的方式

  * 线性渐变

    线性渐变的类型为linear, 他需要配置线性的方向, 通过`x, y, x2, y2`即可进行配置`x, y, x2, y2`, 范围从 0 - 1，相当于在图形包围盒中的百分比，如果 global 为 true，则该四个值是绝对的像素位置
    ...
    ![40](images\echarts\40.png)

  * 径向渐变

    线性渐变的类型为radial, 他需要配置径向的方向, 通过`x, y, r`即可进行配置
...
* 高亮样式
  ...
  那它的使用也非常简单,在emphasis中包裹原先的itemStyle等等
...
* 显示加载动画：通常在获取图表数据之前 显示加载动画

  `mCharts.showLoading()`

* 隐藏加载动画：通常在获取图表数据之后 隐藏加载动画, 显示图表
...
* 创建 Buffer 
  1. Buffer.alloc(length)：创建一个长度为length字节的buffer
  2. Buffer.allocUnsafe(length)：创建一个长度为length字节的buffer，这个buffer中可能存在旧数据，是unsafe的
  3. Buffer.from(content)：通过字符串/数组创建buffer
* Buffer与字符串转化
  * 字符串转化为Buffer：Buffer.from
  * Buffer转化为字符串：buf.toString()
* Buffer的读写
...
4. **createWriteStream 流式写入**
   ...
   * options  选项配置（可选）
   ...
   > 流式写入方式适用于**大文件写入或者频繁写入**的场景, writeFile 适合于写入频率较低的场景
...
dirname 保存着当前文件所在目录的绝对路径，可以使用 __dirname 与文件名拼接成绝对路径
